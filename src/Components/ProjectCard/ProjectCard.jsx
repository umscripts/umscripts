...
const ProjectCard = ({ id, category, image, link, video, name, description }) => {
    ...
    return (
        <div key={id} className='text-sm p-5 border border-gray-300 rounded-lg space-y-3 hover:shadow-custom-shadow'>
            <img src={image} alt={name} className="w-full h-auto max-h-40 object-cover border border-gray-300 rounded-lg overflow-hidden" />
            ...
            <p className='overflow-y-scroll h-36'>{description}</p>
